...
const getPerUnitCost = (row) =>
  toNumberOrDefault(row.perUnitCost ?? row.unitCost ?? row.cost ?? 0, 0);
const getOnHand = (row) =>
  toNumberOrDefault(row.onHand ?? row.totalOnHand ?? row.stock ?? 0, 0);
const getReservedQty = (row) =>
  toNumberOrDefault(row.reservedQty ?? row.totalReserved ?? 0, 0);
const getFreeToUse = (row) => {
  if (row.freeToUse !== undefined && row.freeToUse !== null)
    return toNumberOrDefault(row.freeToUse, 0);
  return getOnHand(row) - getReservedQty(row);
};
...
export default function ProductListPage() {
  ...
  const columns = [
    ...
    },
    {
      key: "perUnitCost",
      label: "Per Unit Cost",
      render: (r) => `₹${getPerUnitCost(r).toFixed(2)}`,
    },
    {
      key: "onHand",
      label: "On Hand",
      render: (r) => {
        const onHand = getOnHand(r);
    ...
    },
    {
      ...
      label: "Free To Use",
      render: (r) => {
        const free = getFreeToUse(r);
        return (
          <span className={clsx(free <= 0 && "text-red-500 font-semibold")}>
            {free}
          </span>
        );
      },
